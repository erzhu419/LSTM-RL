...
\subsection{RE-SAC: Retrieving polynomial bounds}
...
However, RE-SAC leverages the specific duality shown by Xu et al. (Appendix~\ref{app:equivalence}), which maps a specific form of robustness (IPM-based) to Norm Regularization.
\begin{equation}
...
The Lean~4 proof (supplementary \texttt{Proof.lean}) mechanises the finite case
...
the approximation error is bounded by $\frac{1}{1-\gamma}\min_\phi\|Q_\phi - Q^*_{robust}\|_\infty$.
The RE-SAC training loop minimises this residual via gradient descent.  The
finiteness assumption of the Lean proof is therefore a proof-of-concept certificate
...
\begin{enumerate}
    ...
    \end{equation}
    in the worst case (see the 1-state 1-action derivation in the supplementary \texttt{Counterproof.lean}).
    The effective contraction factor becomes $\gamma + \lambda_{\mathrm{ale}}$, which exceeds~1 for any non-trivial aleatoric coefficient, \emph{preventing contraction}. Formally (machine-verified in \texttt{Counterproof.lean}, Theorem~\ref{thm:counterproof}):
    \phantomsection\label{thm:counterproof}%
    \begin{quote}
        \textbf{(Non-contraction, \texttt{Counterproof.lean}: \texttt{T\_bad\_not\_contraction})} Let $\gamma\ge 0$, $\lambda\ge 0$, $\gamma+\lambda\ge 1$. Then $T_{\mathrm{bad}}(q)=(\gamma+\lambda)q$ is not a contraction: $\exists\,q_1,q_2$ s.t.\ $|T_{\mathrm{bad}}(q_1)-T_{\mathrm{bad}}(q_2)|\ge|q_1-q_2|$.
    \end{quote}
...
\midrule
$T_{\mathrm{bad}}$ (Counterproof.lean) & $\lambda\cdot Q$ (Q-dependent) & $\gamma+\lambda$ & $\times$ if $\gamma{+}\lambda\ge 1$ \\
$\mathcal{T}^{REV}_\kappa$ (Proof.lean) & $\kappa$ (fixed scalar) & $\gamma$ & $\checkmark$ ($\gamma<1$) \\
\bottomrule
\end{tabular}
\end{center}
\medskip
...
Our contribution is not a claim of global convergence but a \emph{structural certificate}: the frozen-parameter design is the exact design choice that preserves the per-step contraction, and any Q-dependent alternative provably breaks it (\texttt{Counterproof.lean}).
...
A potential concern is that $\Gamma_{epi}(s,a) = \mathrm{Var}\bigl(\{Q_{\phi'_k}(s,a)\}\bigr)$ depends on Q-functions and could break contraction (cf.\ the counterexample in \texttt{Counterproof.lean}).
...
During the backup step $Q \mapsto \mathcal{T}^{REV}_\kappa Q$, the function $\Gamma_{epi}: \mathcal{S}\times\mathcal{A}\to\mathbb{R}$ is a fixed mapping, independent of $Q$.  The Lean proof reflects this: \texttt{Γ\_epi} is a parameter of type \texttt{S → A → ℝ}, not a function of \texttt{Q}.
...
The empirical stability of RE-SAC (§5) should therefore be understood as the \emph{joint} effect of per-step contraction \emph{plus} standard stabilising mechanisms (target networks, replay buffer, gradient clipping).
