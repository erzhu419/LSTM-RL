...
parser.add_argument('--save_root', type=str, default='q_comparasion/results', help='Base directory for saving models')
...
# Q-Comparison Logging
q_real_log = []
q_pred_log = []

def calculate_q_real(env, trainer, action_dict, bus_id, gamma=0.99, horizon=50):
    snapshot = env.get_snapshot()
    rng_state = np.random.get_state()
    py_rng_state = random.getstate()
    
    current_gamma = args.gamma # Use the gamma from args, usually 0.99
    # The first reward we get is from the transition (s,a)->(s',r).
    # In Q-learning target: r + gamma * Q(s',a').
    # So the immediate reward has discount 1.0 (relative to current step).
    accumulated_discount = 1.0
    G = 0.0
    
    # We need to run step with the provided 'action_dict' first
    temp_action_dict = copy.deepcopy(action_dict)
    
    # We only care about the return for the target 'bus_id'.
    # Note: In this Async env, multiple "steps" (events) might happen.
    # The reward for (s,a) comes when the bus arrives at next station.
    # So we should see 'bus_id' in 'reward_dict' eventually.
    
    target_bus_rewarded = False
    
    for _ in range(horizon):
        state_dict, reward_dict, done = env.step(temp_action_dict)
        
        # Check for rewards
        if bus_id in reward_dict:
            r = reward_dict[bus_id]
            if args.use_reward_scaling:
                 r = trainer.reward_scaling(r)
            
            # The first time we see reward for bus_id, it is the immediate reward R_t for (S_t, A_t).
            # Subsequent rewards are R_{t+1}, etc.
            G += accumulated_discount * r
            accumulated_discount *= gamma
            target_bus_rewarded = True
        
        if done:
            break
            
        # Generate next actions for all agents who need it
        temp_action_dict = {k: None for k in range(env.max_agent_num)}
        for key in state_dict:
             ...
             if args.use_state_norm:
                  state_input = trainer.state_norm(np.array(curr_obs))
             else:
                  state_input = np.array(curr_obs)
                  
             # Deterministic rollout
             a = trainer.policy_net.get_action(torch.from_numpy(state_input).float(), deterministic=True)
             temp_action_dict[key] = a
...
# We will inject the call loop later
...
if __name__ == '__main__':
    if args.train:
        ...
        for eps in range(args.max_episodes):
            ...
            # Use dedicated RNG for Q-comparison trigger to avoid affecting simulation RNG stream
            q_compare_rng = np.random.RandomState(42 + eps)
            if eps != 0:
                env.reset()
            ...
            while not done:
                for key in state_dict:
                    if len(state_dict[key]) == 1:
                        ...
                        # --- Q-Comparison Injection ---
                        # Optimization: Only compare for Bus 0 to reduce overhead (frequency ~1% of Bus 0 steps)
                        # Use dedicated RNG to avoid side effects on global np.random state
                        if args.train and key == 0 and q_compare_rng.rand() < 0.01:
                             t_state = torch.from_numpy(state_input).float().unsqueeze(0).to(device)
                             # action_dict[key] is numpy array of shape (1,).
                             # We want t_action to be (1, 1).
                             # t_action = torch.tensor(action_dict[key], dtype=torch.float32).unsqueeze(0).to(device)
                             # Or simpler:
                             t_action = torch.from_numpy(action_dict[key]).float().unsqueeze(0).to(device)
                             
                             with torch.no_grad():
                                 q_pred = sac_trainer.soft_q_net(t_state, t_action).mean().item()
                             
                             q_real = calculate_q_real(env, sac_trainer, action_dict, key)
                             q_real_log.append(q_real)
                             q_pred_log.append(q_pred)
                             if debug:
                                 print(f"Q-Comparison Bus {key}: Pred {q_pred:.2f} vs Real {q_real:.2f}")
                             
                             # REMOVED GC.COLLECT() - It causes massive slowdown. 
                             # The optimized Station.__deepcopy__ should mitigate memory pressure.
                        # -----------------------------
            ...
            if eps % args.plot_freq == 0:  # plot and model saving interval
                ...
                np.save(os.path.join(logs_path, 'q_real_log.npy'), q_real_log)
                np.save(os.path.join(logs_path, 'q_pred_log.npy'), q_pred_log)
                # -----------------
