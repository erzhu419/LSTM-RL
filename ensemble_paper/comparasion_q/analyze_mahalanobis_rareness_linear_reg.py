...
def get_aligned_error_linear_regression(df, pred_col='q_pred_mean'):
    """Linear Regression alignment (Original method)."""
...
def plot_all_sequential(data_dir, output_dir='analysis_results'):
    ...
    for i, (name, subsample) in enumerate(algos):
        ...
        print(f"[{name.upper()}] Aligning (Linear Regression)...")
        df['error'], params = get_aligned_error_linear_regression(df)
    ...
    ax0.set_title('Mean Q-Value Predictions vs Ground Truth (Linear Regression Alignment)', fontsize=15, fontweight='bold')
    ...
    plt.tight_layout()
    out_path = os.path.join(output_dir, 'comparison_mahalanobis_rareness_reproduced_linear_reg.png')
