...
parser.add_argument('--q_agg_mode', type=str, default='min', choices=['min', 'softmin', 'topk', 'lcb'], help="aggregation mode for ensemble Q (min, softmin, topk, or lcb)")
parser.add_argument('--agg_temperature', type=float, default=1.0, help="temperature for softmin aggregation (ignored otherwise)")
parser.add_argument('--agg_topk', type=int, default=2, help="K for top-k aggregation (ignored otherwise)")
parser.add_argument('--eta', type=float, default=10., help="weight for EDAC gradient diversity regularizer")
...
class SAC_Trainer():
    def __init__(self, env, replay_buffer, hidden_dim, action_range):
        ...
        self.agg_mode = args.q_agg_mode
        self.agg_temperature = max(args.agg_temperature, 1e-6)
        self.agg_topk = max(1, min(args.agg_topk, self.soft_q_net.ensemble_size))
        self.eta = args.eta

    def aggregate_q_values(self, q_values):
        if self.agg_mode == 'min':
            aggregated = q_values.min(dim=0)[0]
        elif self.agg_mode == 'softmin':
            scaled = -q_values / self.agg_temperature
            lse = torch.logsumexp(scaled, dim=0)
            aggregated = -self.agg_temperature * (lse - math.log(q_values.shape[0]))
        elif self.agg_mode == 'topk':
            k = min(self.agg_topk, q_values.shape[0])
            topk_vals, _ = q_values.topk(k, dim=0, largest=False)
            aggregated = topk_vals.mean(dim=0)
        elif self.agg_mode == 'lcb':
            mean = q_values.mean(dim=0)
            std = q_values.std(dim=0, unbiased=False)
            aggregated = mean + args.beta * std
        else:
            raise ValueError(f"Unknown aggregation mode: {self.agg_mode}")
        return aggregated

    def compute_gradient_diversity_loss(self, state, action):
        if self.eta <= 0:
            return torch.zeros(1, device=state.device)

        actions_tile = action.unsqueeze(0).repeat(self.soft_q_net.ensemble_size, 1, 1).clone().detach().requires_grad_(True)
        cat_tensor = state[:, :len(self.soft_q_net.embedding_layer.cat_cols)].long()
        num_tensor = state[:, len(self.soft_q_net.embedding_layer.cat_cols):]
        embedding = self.soft_q_net.embedding_layer(cat_tensor)
        state_emb = torch.cat([embedding, num_tensor], dim=1)
        state_tile = state_emb.unsqueeze(0).repeat(self.soft_q_net.ensemble_size, 1, 1)
        # Vectorized critic expects concatenated state-action
        state_action = torch.cat([state_tile, actions_tile], dim=-1)
        q_preds = self.soft_q_net.critic(state_action).squeeze(-1)

        grads = torch.autograd.grad(q_preds.sum(), actions_tile, retain_graph=True, create_graph=True)[0]
        grads = grads / (grads.norm(p=2, dim=2, keepdim=True) + 1e-10)
        grads = grads.permute(1, 0, 2)  # (batch, ensemble, action_dim)
        sim = torch.matmul(grads, grads.transpose(1, 2))
        mask = torch.eye(sim.size(1), device=sim.device).unsqueeze(0)
        sim = (1 - mask) * sim
        grad_loss = sim.sum(dim=(1, 2)) / (self.soft_q_net.ensemble_size - 1)
    ...
    def compute_q_loss(self, state, action, reward, next_state, done, new_next_action, next_log_prob, reg_norm, gamma):
        predicted_q_value = self.soft_q_net(state, action)  # (ensemble, batch)
        batch_size = predicted_q_value.shape[1]
        reg_expanded = reg_norm.unsqueeze(-1).expand(-1, batch_size)

        with torch.no_grad():
            target_q_next = self.target_soft_q_net(next_state, new_next_action)
            target_adjusted = target_q_next + args.weight_reg * reg_expanded
            target_lower_bound = self.aggregate_q_values(target_adjusted)
            target_lower_bound = target_lower_bound - self.alpha * next_log_prob.detach()
            reward_flat = reward.squeeze(-1)
            done_flat = done.squeeze(-1)
            bellman_target = reward_flat + (1 - done_flat) * gamma * target_lower_bound

        target_q_value = bellman_target.unsqueeze(0).expand_as(predicted_q_value)
        q_value_loss = self.soft_q_criterion(predicted_q_value, target_q_value.detach())

        grad_loss = self.compute_gradient_diversity_loss(state, action)
        loss = q_value_loss + self.eta * grad_loss
        return loss, predicted_q_value, grad_loss
    ...
    def compute_policy_loss(self, state, action, new_action, log_prob, reg_norm):
        ...
        # Freeze critic params during actor forward so that backward
        # only flows through actions, avoiding in-place version issues
        critic_params = [p for p in self.soft_q_net.parameters()]
        prev_requires = [p.requires_grad for p in critic_params]
        for p in critic_params:
            p.requires_grad_(False)
        q_heads = self.soft_q_net(state, new_action) + args.weight_reg * reg_expanded
        for p, flag in zip(critic_params, prev_requires):
            p.requires_grad_(flag)
        q_lb = self.aggregate_q_values(q_heads)

        entropy_term = self.alpha * log_prob
        q_loss = -(q_lb - entropy_term).mean()

        bc_loss = F.mse_loss(new_action, action)
        loss = q_loss + args.beta_bc * bc_loss

        q_std = q_heads.std(dim=0)

        return loss, q_lb.detach(), q_std.detach()
    ...
    def update(self, batch_size, training_steps, reward_scale=10., auto_entropy=True, target_entropy=-2, gamma=0.99, soft_tau=1e-2):
        global q_values, reg_norms, log_probs, alpha_values, grad_losses, q_stds
        ...
        reg_norm_target = self.compute_reg_norm(self.target_soft_q_net)
        # Detach so actor loss doesn't depend on critic params via reg term
        reg_norm_online = self.compute_reg_norm(self.soft_q_net).detach()

        q_value_loss, predicted_q_value, grad_loss = self.compute_q_loss(state, action, reward, next_state, done, new_next_action, next_log_prob, reg_norm_target, gamma)
        ...
        if training_steps % args.critic_actor_ratio == 0:
            policy_loss, q_lb, q_std = self.compute_policy_loss(state, action, new_action, log_prob, reg_norm_online)
        ...
        reg_norms.append(args.weight_reg * reg_norm_online.mean().item())
...
def plot(rewards):
    ...
    plt.plot(grad_losses_episode, label="Gradient Diversity")
...
model_path = f"./model/sac_ensemble_original/replay_buffer_size_{args.replay_buffer_size}/critic_actor_ratio_{args.critic_actor_ratio}/maximum_alpha_{args.maximum_alpha}/weight_reg_{args.weight_reg}"
    ...
    if args.train:
        ...
        for eps in range(args.max_episodes):
            ...
            if training_steps > 0:
                ...
                grad_losses_episode.append(np.mean(grad_losses[-training_steps:]))
                q_stds_episode.append(np.mean(q_stds[-training_steps:])) if len(q_stds) > 0 else q_stds_episode.append(0)

                # 清理per-step的监控数组，防止内存无限增长
                # 只保留最近的数据用于episode统计
                max_keep = 10000  # 保留最近10000个step的数据
                if len(reg_norms) > max_keep:
                    reg_norms = reg_norms[-max_keep:]
                    log_probs = log_probs[-max_keep:]
                    alpha_values = alpha_values[-max_keep:]
                    grad_losses = grad_losses[-max_keep:]
                    q_stds = q_stds[-max_keep:]
                    # q_values也需要清理
                    if q_values.shape[1] > max_keep:
                        q_values = q_values[:, -max_keep:]
            else:
                # 如果没有训练，添加占位值
                if len(q_values_episode) == 0:
                    q_values_episode = np.zeros((10, 1), dtype=np.float32)
                else:
                    q_values_episode = np.concatenate((q_values_episode, np.zeros((10, 1), dtype=np.float32)), axis=1)
                reg_norms_episode.append(0)
                log_probs_episode.append(0)
                alpha_values_episode.append(0)
                grad_losses_episode.append(0)
