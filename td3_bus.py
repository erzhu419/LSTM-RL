"""
Twin Delayed Deep Deterministic Policy Gradient (TD3) for the bus holding control environment
...
parser = argparse.ArgumentParser(description='TD3 baseline for bus holding control.')
...
parser.add_argument("--noise_clip", type=float, default=0.5, help="Clip range for target policy smoothing noise")
parser.add_argument("--policy_delay", type=int, default=2, help="Delay (in steps) for policy/target updates")
parser.add_argument('--save_root', type=str, default='.', help='Base directory for saving outputs')
parser.add_argument('--run_name', type=str, default='td3_baseline', help='Identifier appended to save directories')
...
MODEL_PREFIX = os.path.join(MODEL_DIR, 'td3_bus')
...
class TD3Trainer:
    def __init__(self, env, replay_buffer, hidden_dim, action_range, action_dim, embedding_mode,
        ...
        self.critic1 = CriticNetwork(state_dim, action_dim, hidden_dim, embedding_template.clone()).to(device)
        self.critic2 = CriticNetwork(state_dim, action_dim, hidden_dim, embedding_template.clone()).to(device)
        self.critic1_target = CriticNetwork(state_dim, action_dim, hidden_dim, embedding_template.clone()).to(device)
        self.critic2_target = CriticNetwork(state_dim, action_dim, hidden_dim, embedding_template.clone()).to(device)
    ...
    def update(self, batch_size, reward_scale=1.0, use_gradient_clip=True):
        ...
        with torch.no_grad():
            noise = torch.normal(mean=0.0, std=self.policy_noise, size=action.shape, device=device)
            noise = noise.clamp(-self.noise_clip, self.noise_clip)
            next_action = (self.actor_target(next_state) + noise).clamp(0.0, self.actor.action_range)
        ...
        critic_loss1 = self.criterion(current_q1, target_q.detach())
        critic_loss2 = self.criterion(current_q2, target_q.detach())
    ...
    def save_model(self, path_prefix):
        ...
        torch.save(self.critic1.state_dict(), path_prefix + '_critic1')
        torch.save(self.critic2.state_dict(), path_prefix + '_critic2')
        torch.save(self.actor_target.state_dict(), path_prefix + '_actor_target')
        torch.save(self.critic1_target.state_dict(), path_prefix + '_critic1_target')
        torch.save(self.critic2_target.state_dict(), path_prefix + '_critic2_target')

    def load_model(self, path_prefix):
        actor_path = path_prefix + '_actor'
        critic1_path = path_prefix + '_critic1'
        critic2_path = path_prefix + '_critic2'
        actor_target_path = path_prefix + '_actor_target'
        critic1_target_path = path_prefix + '_critic1_target'
        critic2_target_path = path_prefix + '_critic2_target'

        print(f"Loading actor weights from {actor_path}")
        self.actor.load_state_dict(_safe_torch_load(actor_path))
        print(f"Loading critic1 weights from {critic1_path}")
        self.critic1.load_state_dict(_safe_torch_load(critic1_path))
        print(f"Loading critic2 weights from {critic2_path}")
        self.critic2.load_state_dict(_safe_torch_load(critic2_path))
        ...
        if os.path.exists(critic1_target_path):
            print(f"Loading critic1 target weights from {critic1_target_path}")
            self.critic1_target.load_state_dict(_safe_torch_load(critic1_target_path))
        else:
            self.critic1_target.load_state_dict(self.critic1.state_dict())
        if os.path.exists(critic2_target_path):
            print(f"Loading critic2 target weights from {critic2_target_path}")
            self.critic2_target.load_state_dict(_safe_torch_load(critic2_target_path))
        else:
            self.critic2_target.load_state_dict(self.critic2.state_dict())

        # ensure training mode for optimisable nets
...
def plot(rewards, critic_losses1, critic_losses2, actor_losses):
    ...
    plt.subplot(1, 2, 2)
    plt.plot(critic_losses1, label="Critic1 Loss")
    plt.plot(critic_losses2, label="Critic2 Loss")
    if actor_losses:
        plt.plot(actor_losses, label="Actor Loss")
    plt.legend()
    plt.title("Loss Curves")

    plt.savefig(os.path.join(PIC_DIR, f'td3_training_{len(rewards)}.png'))
    plt.close()
...
if args.resume:
    ...
    if latest_episode is not None:
        ...
        critic_losses1 = _load_history(Path(LOG_DIR) / 'critic1_losses.npy')
        critic_losses2 = _load_history(Path(LOG_DIR) / 'critic2_losses.npy')
        ...
        critic_losses1 = list(critic_losses1)
        critic_losses2 = list(critic_losses2)
    ...
    if args.train:
        ...
        for eps in range(start_episode, args.max_episodes):
            ...
            while not done:
                ...
                if len(replay_buffer) > args.batch_size and len(replay_buffer) % args.training_freq == 0 and step_trained != step:
                    ...
                    for _ in range(update_itr):
                        ...
                        training_steps += 1
                        critic_losses1.append(update_stats['critic_loss1'])
                        critic_losses2.append(update_stats['critic_loss2'])
                        if update_stats['actor_loss'] is not None:
            ...
            if eps % args.plot_freq == 0:
                plot(rewards, critic_losses1, critic_losses2, actor_losses)
                np.save(os.path.join(LOG_DIR, 'rewards.npy'), rewards)
                np.save(os.path.join(LOG_DIR, 'critic1_losses.npy'), critic_losses1)
                np.save(os.path.join(LOG_DIR, 'critic2_losses.npy'), critic_losses2)
                ...
                trainer.save_model(os.path.join(LOG_DIR, f'td3_episode_{eps}'))
