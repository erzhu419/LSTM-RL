...
class Station(object):
    ...
    def __deepcopy__(self, memo):
        ...
        # Copy attributes
        # Shallow copy immutable/shared data
        result.station_type = self.station_type
        result.station_id = self.station_id
        result.station_name = self.station_name
        result.direction = self.direction
        result.od = self.od  # Share the OD matrix (read-only dict)
        
        # Deepcopy mutable state
        result.waiting_passengers = copy.deepcopy(self.waiting_passengers, memo)
        result.total_passenger = copy.deepcopy(self.total_passenger, memo)
