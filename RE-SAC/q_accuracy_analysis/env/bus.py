...
class Bus(object):
    ...
    def drive(self, current_time, action, bus_all, debug):
        if debug and current_time % 100 == 0:
            print(f"[DEBUG] Bus {self.bus_id} (Trip {self.trip_id}): Speed={self.current_speed:.2f}, AbsDist={self.absolute_distance:.1f}, InStation={self.in_station}, NextSt={self.next_station.station_name}")
        ...
        if self.next_station_dis <= self.current_speed and not self.holding and not self.dwelling:
            ...
            if self.holding_time <= 1:
                ...
                if not self.held:
                    ...
                    if self.next_station in self.effective_station:
