...
class Route(object):
    ...
    def route_update(self, current_time, effective_period):
        ...
        try:
            mean_speed = self.speed_history.loc[current_hour]
        except KeyError:
            # Fallback: use the last available speed if exact time is missing
            mean_speed = self.speed_history.iloc[-1] if not self.speed_history.empty else 10.0
            
        v = np.clip(math.log(random.lognormvariate(mean_speed, self.sigma)), 2, 15)
