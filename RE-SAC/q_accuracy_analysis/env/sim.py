...
class env_bus(object):
    ...
    def __init__(self, path, debug=False, render=False, route_sigma=1.5):
        ...
        self.timetable_set = self.timetable_set.sort_values(by=['launch_time', 'direction']).reset_index(drop=True)
    ...
    def get_snapshot(self):
        # Bundle objects to preserve references during deepcopy (Crucial for Bus->Station links)
        # We explicitly bundle all interconnected lists/dicts
        critical_objects = (self.bus_all, self.stations, self.routes, self.timetables, 
                           self.state, self.reward, self.route_state, self.action_dict)
        
        snapshot = {
            'current_time': self.current_time,
            'critical_objects': copy.deepcopy(critical_objects),
            'done': self.done,
            'bus_id': self.bus_id,
            'rng_state': np.random.get_state(),  # Save Numpy RNG state
            'py_rng_state': random.getstate()    # Save Python RNG state (for Route speeds)
        }
        return snapshot
    ...
    def load_snapshot(self, snapshot):
        ...
        # Unpack bundled objects
        (self.bus_all, self.stations, self.routes, self.timetables, 
         self.state, self.reward, self.route_state, self.action_dict) = snapshot['critical_objects']
        
        self.done = snapshot['done']
        self.bus_id = snapshot['bus_id']
        
        # Restore RNG state (Crucial for Side Effect Free rollout)
        if 'rng_state' in snapshot:
            np.random.set_state(snapshot['rng_state'])
        if 'py_rng_state' in snapshot:
            random.setstate(snapshot['py_rng_state'])
    ...
    def reset(self, snapshot=None):
        ...
        # self.reward = defaultdict(float) # We will reset this every step anyway
        self.done = False
    ...
    def step(self, action, debug=False, render=False):
        ...
        # Reset rewards at the start of each step to prevent stickiness
